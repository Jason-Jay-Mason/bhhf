import { styled } from "@linaria/react";
import { theme, spacing } from "../styles/theme";
import { usePopupSource, usePopupStatus, useSetPopupSource, usePopupToggle } from "../hooks/usePopUpModal";

//#region styles
const div = {};
div.popupStuff = styled.div`
  position: fixed;
  top: 0;
  left: 0;
  height: 100vh;
  width: 100vw;
  background-color: #0000008f;
  z-index: 10;
  display: ${(props) => (props.isVisible ? "flex" : "none")};
  flex-direction: column;
  div {
    margin: auto;
  }
  iframe {
    width: 890px;
    height: 500px;
    display: block;
    @media ${theme.breakPoints.lrg} {
      width: 100vw;
      height: 500px;
    }
    @media ${theme.breakPoints.sml} {
      width: 100vw;
      height: 50vh;
    }
  }
  .close {
    cursor: pointer;
    top: 50px;
    left: 50px;
    margin-right: auto;
    display: flex;
    align-items: center;
    flex-direction: row;
    position: absolute;
    height: 20px;
    p {
      padding-left: 10px;
      color: white;
    }
  }
`;

//#endregion

const PopupVideo = () => {
  const popupSource = usePopupSource();
  const popupStatus = usePopupStatus();
  const popupToggle = usePopupToggle();
  return (
    <div.popupStuff
      isVisible={popupStatus}
      onClick={() => {
        popupToggle(false);
      }}
    >
      <div className="close">
        <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" color="white" height="30px" width="30px" xmlns="http://www.w3.org/2000/svg">
          <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm165.4 618.2l-66-.3L512 563.4l-99.3 118.4-66.1.3c-4.4 0-8-3.5-8-8 0-1.9.7-3.7 1.9-5.2l130.1-155L340.5 359a8.32 8.32 0 0 1-1.9-5.2c0-4.4 3.6-8 8-8l66.1.3L512 464.6l99.3-118.4 66-.3c4.4 0 8 3.5 8 8 0 1.9-.7 3.7-1.9 5.2L553.5 514l130 155c1.2 1.5 1.9 3.3 1.9 5.2 0 4.4-3.6 8-8 8z"></path>
        </svg>
        <p>CLOSE</p>
      </div>
      <div>
        <iframe modestbranding="1" disablekb="1" color="white" src={popupSource} title="Broken Heart Horse Farm" frameBorder="0" allowFullScreen="1" playsInline="1"></iframe>
      </div>
    </div.popupStuff>
  );
};

export default PopupVideo;
